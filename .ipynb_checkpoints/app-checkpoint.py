...
def load_data():
    ...
    try:
        ...
        st.error("Please check if all required files are in the Dataset folder!")
...
symptoms_dict = {
    ...
    'distention_of_abdomen': 115, 'history_of_alcohol_consumption': 116, 'fluid_overload': 117,
...
def get_predicted_value(patient_symptoms, model):
    # Create a zero vector with 132 features (matching the training data)
...
def main():
    ...
    st.markdown("### Get personalized health recommendations based on your symptoms")
    ...
    selected_symptoms = st.sidebar.multiselect(
        ...
        help="Select multiple symptoms that you are experiencing"
    )
    ...
    if st.sidebar.button("Get Diagnosis"):
        if len(selected_symptoms) > 0:
            with st.spinner("Analyzing symptoms..."):
                ...
                desc, pre, med, die, wrkout = helper(predicted_disease, description, precautions, medications, diets, workout)
                ...
                with col1:
                    ...
                    st.markdown(f"<div class='diagnosis-box'><h3>{predicted_disease}</h3></div>", unsafe_allow_html=True)
                ...
                medication_col1, medication_col2 = st.columns(2)
                with medication_col1:
                    for m in med:
                        st.markdown(f"- {m}")

                st.markdown("### 🏃‍♂️ Recommended Workout/Activities")
